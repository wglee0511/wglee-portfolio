...
플랫폼 경계 영역(홈 위젯·Wear OS·백그라운드 위치 알림)을 직접 설계하고 구현합니다. 코드 분석 → 계획 수립 → 결과 검증 사이클로 AI를 설계 파트너 삼아 낯선 네이티브 영역도 단독으로 구현합니다.

기획부터 배포까지 제품의 전 과정을 경험합니다. 2026년 상반기에만 복약 기록 앱(**Cadence**), 위치 리마인더 앱(**스팟 스테이플러**), 주유소 추천 웹(**찍고가**)을 PRD 작성부터 배포까지 완주했습니다.
