...
기획·설계·구현·배포까지 혼자 끝내는 사이클을 반복합니다. 2026년 상반기에만 세 개의 제품을 PRD 작성부터 배포까지 단독으로 완주했습니다. GLP-1 복약 기록 앱(**Cadence**)은 SQLite를 진실의 원천으로 둔 오프라인 퍼스트 구조와 진료용 PDF 리포트를, 위치 기반 리마인더 앱(**스팟 스테이플러**)은 앱을 종료해도 동작하는 지오펜싱과 멱등성을 보장하는 오프라인 큐를, 주유소 추천 웹(**찍고가**)은 런타임 의존성 없이 경로↔주유소 매칭 알고리즘과 좌표계 변환을 직접 구현했습니다.
